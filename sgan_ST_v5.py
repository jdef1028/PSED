...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # output layer
        conv5 = tf.layers.conv2d(lrelu4, 1, [8, 8], strides=(1, 1), padding='valid')
...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    size_figure_grid = 5
    fixed_z_ = np.random.normal(0, 1, (size_figure_grid*size_figure_grid,)+feature_vector_dim)
    fixed_vf_ = np.random.uniform(-0.5, 0.5, (size_figure_grid*size_figure_grid,)+feature_vector_dim)
...
def getVF(G_z, k=0.4):
    mean_vector = tf.reduce_mean(G_z, axis=[1,2,3]) + \
                  0.4*tf.sqrt(tf.reduce_sum(G_z ** 2, axis=[1,2,3])/height/width - tf.reduce_mean(G_z, axis=[1,2,3]) ** 2)
    #print mean_vector.shape
    mean_mask = tf.expand_dims(mean_vector, axis=1)
    mean_mask = tf.expand_dims(mean_mask, axis=2)
    mean_mask = tf.tile(mean_mask, (1, height, width))
    mean_mask = tf.expand_dims(mean_mask, axis=3)
    #print G_z.shape, mean_mask.shape

    binary = tf.cast(G_z - mean_mask, tf.int8)
    return tf.reduce_mean(binary, axis=(1,2,3))  
...
nowtime = 'exp_56_gray_1'
D_steps = 3
G_steps = 1
snapshot_interval = 1000 # save image generated by generator 
...
# vf related calculation
vf_collection = np.sum(img_collection, axis=1) / float(img_dim[0]) / float(img_dim[1])


del img_collection_data
# variables : input
x = tf.placeholder(tf.float32, shape=(None,) + img_dim, name='x')
z = tf.placeholder(tf.float32, shape=(None,) + feature_vector_dim[:-1]+(feature_vector_dim[-1]+1,), name='z')
...
vf_pred = getVF(G_z)
vf_label = z[:, 0, 0, -1]
vf_loss = tf.losses.mean_squared_error(labels=vf_label, predictions=vf_pred)
...
G_loss = G_loss + 0.001*cl_loss + 0.01*sl_loss + 0.1*vf_loss
