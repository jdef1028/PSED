...
def generator(x, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        deconv0 = tf.layers.conv2d(x, 64, [4, 4], strides=(2, 2), padding='same')
        derelu0 = relu(tf.layers.batch_normalization(deconv0, training=isTrain))

        deconv1 = tf.layers.conv2d(derelu0, 128, [4, 4], strides=(2, 2), padding='same')
        derelu1 = relu(tf.layers.batch_normalization(deconv1, training=isTrain))

        # 2nd hidden layer
        deconv2 = tf.layers.conv2d(derelu1, 256, [4, 4], strides=(2, 2), padding='same')
        derelu2 = relu(tf.layers.batch_normalization(deconv2, training=isTrain))

        # 3rd hidden layer
        deconv3 = tf.layers.conv2d(derelu2, 512, [4, 4], strides=(2, 2), padding='same')
        derelu3 = relu(tf.layers.batch_normalization(deconv3, training=isTrain))

        # 4th hidden layer
        deconv4 = tf.layers.conv2d(derelu3, 1024, [4, 4], strides=(2, 2), padding='same')
        derelu4 = relu(tf.layers.batch_normalization(deconv4, training=isTrain))

        # output layer
        deconv5 = tf.layers.conv2d(derelu4, 1, [4, 4], strides=(1, 1), padding='valid')

        # 1st hidden layer
        conv1 = tf.layers.conv2d_transpose(deconv5, 1024, [4, 4], strides=(1, 1), padding='valid')
        ...
        # 3rd hidden layer
        conv3 = tf.layers.conv2d_transpose(relu2, 256, [4, 4], strides=(2, 2), padding='same')
        ...
        conv5 = tf.layers.conv2d_transpose(relu4, 64, [4, 4], strides=(2, 2), padding='same')
        relu5 = relu(tf.layers.batch_normalization(conv5, training=isTrain))
        # output layer

        conv6 = tf.layers.conv2d_transpose(relu5, 1, [4, 4], strides=(2, 2), padding='same')
        o = tf.nn.tanh(conv6)
...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        # 1st hidden layer
        conv0 = tf.layers.conv2d(x, 64, [4, 4], strides=(2, 2), padding='same')
...
D_steps = 3
G_steps = 1
snapshot_interval = 10 # save image generated by generator 
start_iter = 100 # the start num of iteration for ploting loss
...
z = tf.placeholder(tf.float32, shape=(None, 128, 128, 1))
isTrain = tf.placeholder(dtype=tf.bool)

# MNIST resize and normalization
train_set_orig = img_collection.reshape(((len(img_collection),)+ img_dim))
# train_set = tf.image.resize_images(mnist.train.images, [64, 64]).eval()
train_set_orig = (train_set_orig - 0.5) / 0.5  # normalization; range: -1 ~ 1
style_array = np.repeat(train_set_orig, 3, axis=-1)
# style_array = np.zeros([len(img_collection),height,width,3])
# style_array[:,:,:,0]=(img_collection.reshape(len(img_collection),height,width))
# style_array[:,:,:,1]=(img_collection.reshape(len(img_collection),height,width))
# style_array[:,:,:,2]=(img_collection.reshape(len(img_collection),height,width))
trainID = random.sample(range(img_collection.shape[0]), batch_size)
x_fix = train_set_orig[trainID,:,:,:]
del img_collection
...
######## style transfer ########
combination_image_temp=tf.reshape(G_z,[batch_size, height, width, 1])*1
combination_image = tf.concat([combination_image_temp, combination_image_temp,combination_image_temp], 3)
style_image = tf.placeholder(tf.float32, shape=(batch_size,height,width,3))
...
for epoch in range(train_epoch):
    ...
    # for iter in range(len(train_set) // batch_size):
        ...
        loss_d_, _ = sess.run([D_loss, D_optim], {x: x_, z: x_, isTrain: True})
    ...
    for G_i in xrange(G_steps):
        ...
        # z_ = np.random.normal(0, 1, (batch_size, 1, 1, 100))
        loss_g_, _ = sess.run([G_loss, G_optim], {z: x_, x: x_, style_image:style_image_input, isTrain: True})
    ...
    if epoch % snapshot_interval in [snapshot_interval-2, snapshot_interval-1,0]:
        ...
        # print (fixed_p)
        show_result(x_fix,(epoch + 1), save=True, path=fixed_p)
