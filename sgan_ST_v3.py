...
def generator(x, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        # 1st hidden layer
        conv1 = tf.layers.conv2d_transpose(x, filter_num[0], [4, 4], strides=(1, 1), padding='valid')
...
def discriminator(x, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        conv1 = tf.layers.conv2d(lrelu0, filter_num[3], [4, 4], strides=(2, 2), padding='same')
...
# training parameters
filter_num = [256, 128, 64, 32, 16] # five numbers from big to small
batch_size = 30
lr = 0.0005
train_epoch = int(5e4+1)
img_dim = (128,128,1)
feature_vector_dim = (1, 1, 100)
...
z = tf.placeholder(tf.float32, shape=(None,) + feature_vector_dim)
isTrain = tf.placeholder(dtype=tf.bool)
fixed_z_ = np.random.normal(0, 1, (25,)+feature_vector_dim)
